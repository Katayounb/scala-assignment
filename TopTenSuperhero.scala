...
/** Find To Ten superhero with the most co-appearances. */
object TopTenSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    val populare = flipped.sortByKey(false)
    val popular = populare.collect()
    val TenPopular = popular.take(10)
    //TenPopular.foreach(println)
   
    for (topten <- TenPopular) {
       val mostPopularName = namesRdd.lookup(topten._2)(0)
       // Print out our answer!
      println(s"$mostPopularName is one the most popular superhero with ${topten._1} co-appearances.") 
    }
  }
}
